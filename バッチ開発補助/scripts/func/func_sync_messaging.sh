...
####################################################################################
#  Script name  : func_sync_messaging.sh
#  Description  : メッセージ同期応答
#                 Javaで実装されたメッセージ同期応答（被仕向け）プログラムを実行する
#  Server type  : MQ server
#  User         : mquser
#  Usage        : func_sync_messaging.sh JOB_ID REQUEST_PATH USER_ID [PARAM_MAP_ENTRY]... [PARAM_LIST_ITEM]...
#                     JOB_ID
#                       ジョブID
#                     REQUEST_PATH
#                       リクエストパス
...
### 実行オプションの生成 ###
NAB_OPT="-classpath ${RUNTIME_CLASSPATH} ${SYNC_MSG_OPT} ${INP_NAB_OPT} "
...
### Mainクラス引数を設定 ###
MAIN_ARGS="-diConfig ${SYNC_MSG_DI_CONF}"
