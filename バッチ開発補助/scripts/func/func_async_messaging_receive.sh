...
####################################################################################
#  Script name  : func_async_messaging_receive.sh
#  Description  : メッセージ無応答・ディレード処理（被仕向け）プログラムを実行する
#  Server type  : MQ Server
#  User         : mquser
#  Usage        : func_async_messaging_receive.sh JOB_ID REQUEST_PATH USER_ID [PARAM_MAP_ENTRY]... [PARAM_LIST_ITEM]...
#                     JOB_ID
#                       ジョブID
#                     REQUEST_PATH
#                       リクエストパス
...
### 実行オプションの生成 ###
NAB_OPT="-classpath ${RUNTIME_CLASSPATH} ${ASYNC_MSG_RCV_OPT} ${INP_NAB_OPT} "
...
### Mainクラス引数を設定 ###
MAIN_ARGS="-diConfig ${ASYNC_MSG_RCV_DI_CONF}"
